...
def output_results(bigwig_names, datagen_bed, model, output_dir):
  ...
  for i, bigwig_name in enumerate(bigwig_names):
    np.save(output_dir + '/' + bigwig_name +
            '_meansignal.npy', motifs_bws[:, :, i])
  ...
  fwd_timedistributed, rev_timedistributed = timedistributed_layer.get_output_at(
      0), timedistributed_layer.get_output_at(1)
  intermediate_layer_model = Model(input=model.input, output=[
                                   fwd_timedistributed, rev_timedistributed])
  ...
  while num_seqs_processed < num_seqs:
    ...
    # N x M matrix, where M is the number of motifs
    max_inds = z[0].argmax(axis=1)
